        ...
        if top5SoldItems :
            ...
            if hasTopSoldItems:
                if 0 < len(top5SoldItems) :
                    soldItem1 = int(float(top5SoldItems[0].text.replace("£", "")))
                if 1 < len(top5SoldItems) :
                    soldItem2 = int(float(top5SoldItems[1].text.replace("£", "")))
                if 2 < len(top5SoldItems) : 
                    soldItem3 = int(float(top5SoldItems[2].text.replace("£", "")))
                if 3 < len(top5SoldItems) : 
                    soldItem4 = int(float(top5SoldItems[3].text.replace("£", "")))
                if 4 < len(top5SoldItems) :
                    soldItem5 = int(float(top5SoldItems[4].text.replace("£", "")))
...
with open('ebayResults.csv', 'wb') as myfile:
    ...
    for row in reader:
        if index >0 :
            ...
            postcode = row[6]
            itemAsDescribedCount = row[7].replace(",",'')

            if itemAsDescribedCount == '' :
                itemAsDescribedCount = 0

            itemAsDescribedRating = row[8]
            communicationCount = row[9].replace(",",'')

            if communicationCount == '' :
                communicationCount = 0

            communicationRating = row[10]

            dispatchTimeCount = row[11].replace(",",'')

            if dispatchTimeCount == '' :
                dispatchTimeCount = 0

            dispatchTimeRating = row[12]
            postageCount = row[13].replace(",",'')

            if postageCount == '' :
                postageCount = 0

            postageRating = row[14]
            soldItem1 = row[15].replace("£",'')
            soldItem1 = soldItem1.replace(",",'')

            if soldItem1 == '' :
                soldItem1 = 0

            soldItem2 = row[16].replace("£",'')
            soldItem2 = soldItem2.replace(",",'')
            
            if soldItem2 == '' :
                soldItem2 = 0

            soldItem3 = row[17].replace("£",'')
            soldItem3 = soldItem3.replace(",",'')

            if soldItem3 == '' :
                soldItem3 = 0

            soldItem4 = row[18].replace("£",'')
            soldItem4 = soldItem4.replace(",",'')

            if soldItem4 == '' :
                soldItem4 = 0

            soldItem5 = row[19].replace("£",'')
            soldItem5 = 0
            memberSince = row[20]
            ebayShopId = row[21]
            category = 'first_batch'
            headlineCategory = 'first_batch'

            if row[22] :
                if 'Yes' in row[22]:
                    importedToHubspot = True
            else :
                importedToHubspot = False

            print str(index) + "-------"+ shopUrl
            print ""
