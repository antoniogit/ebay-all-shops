...
def isThereAnotherShopIdInDb(ebayShopId, categoryUrl) :
...
def getProblematics(fileName):
    ...
    for row in reader:
        if index > 0:
            if row[0] not in problematics:
                problematics.append(row[0])
        index += 1
    return problematics
...
with open('ebayProblematicCategories.csv', 'wb') as myfile:
    ...
    problematics = getProblematics('Problematics.csv')
    hasPages = True
    currentPage = 1
    startPage = 0
    ...
    index = 0
    for row2 in reader:
        if index > 0 and row2[3] in problematics:
                                ...
                                for pageItem in pageItems:
                                    ...
                                    if itemSellerSoup :
                                        ...
                                        if shopIds:
                                            ...
                                            if isThereAnotherShopIdInDb(shopId, categoryUrl) == False:
                                                visited = True
